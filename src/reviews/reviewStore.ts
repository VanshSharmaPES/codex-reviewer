...
export interface ReviewHistoryEntry { id: string; owner: string; repo: string; prNumber: number; status: 'passed' | 'failed' | 'partial'; violations: number; filesAnalyzed?: number; durationMs?: number; provider?: string; profileFingerprint?: string; createdAt: string; }
